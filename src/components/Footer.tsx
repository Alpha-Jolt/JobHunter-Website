import type { MouseEvent } from 'react'
import type { Page } from '../App'

interface Props {
  navigate: (p: Page) => void
}

const socials = [
  {
    label: 'GitHub',
    href: 'https://github.com/alpha-jolt/jobhunter',
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        <path d="M12 2C6.477 2 2 6.477 2 12c0 4.418 2.865 8.166 6.839 9.489.5.092.682-.217.682-.482 0-.237-.009-.868-.013-1.703-2.782.604-3.369-1.342-3.369-1.342-.454-1.155-1.11-1.463-1.11-1.463-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.578 9.578 0 0 1 12 6.836a9.59 9.59 0 0 1 2.504.337c1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.202 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.578.688.48C19.138 20.163 22 16.418 22 12c0-5.523-4.477-10-10-10z" />
      </svg>
    ),
  },
  {
    label: 'Instagram',
    href: 'https://instagram.com/my_jobhunter',
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z" />
      </svg>
    ),
  },
  {
    label: 'LinkedIn',
    href: 'https://linkedin.com/company/my-jobhunter',
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
      </svg>
    ),
  },
  {
    label: 'X',
    href: 'https://x.com/myjobhunterhq',
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.738l7.73-8.835L1.254 2.25H8.08l4.261 5.632 5.903-5.632zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
      </svg>
    ),
  },
] as const

export default function Footer({ navigate }: Props) {
  const getHref = (p: Page) => {
    if (typeof window === 'undefined') return p === 'home' ? '/' : `/${p}`
    const isSubdir = window.location.pathname.startsWith('/JobHunter-Website')
    const prefix = isSubdir ? '/JobHunter-Website' : ''
    return p === 'home' ? `${prefix}/` : `${prefix}/${p}`
  }

  const handleNavigate = (e: MouseEvent, p: Page) => {
    e.preventDefault()
    navigate(p)
  }

  return (
    <footer className="footer">
      <div className="footer-inner">
        <div className="footer-brand-block">
          <a
            href={getHref('home')}
            className="footer-logo"
            onClick={(e) => handleNavigate(e, 'home')}
            aria-label="JobHunter home"
          >
            Job<span>Hunter</span>
          </a>
          <p className="footer-tagline">
            Human-in-the-loop job search. Built in public by VSRN in Coimbatore.
          </p>
          <div className="footer-social-list">
            {socials.map((s) => (
              <a
                key={s.label}
                href={s.href}
                target="_blank"
                rel="noopener noreferrer"
                className="footer-social"
                aria-label={s.label}
              >
                {s.icon}
              </a>
            ))}
          </div>
        </div>

        <div className="footer-top">
          <div className="footer-col">
            <h4 className="footer-col-title">Product</h4>
            <ul className="footer-col-links">
              <li>
                <a href={getHref('features')} className="footer-link-item" onClick={(e) => handleNavigate(e, 'features')}>
                  Features
                </a>
              </li>
              <li>
                <a href={getHref('for-who')} className="footer-link-item" onClick={(e) => handleNavigate(e, 'for-who')}>
                  Who it&apos;s for
                </a>
              </li>
              <li>
                <a href={getHref('referral')} className="footer-link-item" onClick={(e) => handleNavigate(e, 'referral')}>
                  Referral
                </a>
              </li>
              <li>
                <a href={getHref('about')} className="footer-link-item" onClick={(e) => handleNavigate(e, 'about')}>
                  About
                </a>
              </li>
            </ul>
          </div>

          <div className="footer-col">
            <h4 className="footer-col-title">Resources</h4>
            <ul className="footer-col-links">
              <li>
                <a href={getHref('faq')} className="footer-link-item" onClick={(e) => handleNavigate(e, 'faq')}>
                  FAQ
                </a>
              </li>
            </ul>
          </div>

          <div className="footer-col">
            <h4 className="footer-col-title">Legal</h4>
            <ul className="footer-col-links">
              <li>
                <a href={getHref('terms-of-service')} className="footer-link-item" onClick={(e) => handleNavigate(e, 'terms-of-service')}>
                  Terms of Service
                </a>
              </li>
              <li>
                <a href={getHref('privacy-policy')} className="footer-link-item" onClick={(e) => handleNavigate(e, 'privacy-policy')}>
                  Privacy Policy
                </a>
              </li>
              <li>
                <a href={getHref('refund-policy')} className="footer-link-item" onClick={(e) => handleNavigate(e, 'refund-policy')}>
                  Refund Policy
                </a>
              </li>
            </ul>
          </div>

          <div className="footer-col">
            <h4 className="footer-col-title">Contact</h4>
            <ul className="footer-col-links contact-info">
              <li>
                <span className="contact-label">Support</span>
                <a href="mailto:support@myjobhunter.in" className="contact-value">support@myjobhunter.in</a>
              </li>
              <li>
                <span className="contact-label">Legal</span>
                <a href="mailto:legal@myjobhunter.in" className="contact-value">legal@myjobhunter.in</a>
              </li>
            </ul>
          </div>
        </div>

        <div className="footer-bottom">
          <p className="footer-copy">
            © {new Date().getFullYear()} JobHunter · a VSRN product · Coimbatore
          </p>
          <span className="footer-status">
            <span className="footer-status-dot" aria-hidden="true" />
            Building in public — Phase 0
          </span>
        </div>
      </div>
    </footer>
  )
}
